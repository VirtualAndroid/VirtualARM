...
GlobalStubs::GlobalStubs(SharedPtr<Instance> instance) : instance_{instance}, context_reg_{
    ...
    abi_switch_host_to_guest_ = code_memory_ + 512 * 2;
    ...
    BuildABISwitchGuestToHost();
    BuildABISwitchHostToGuest();
}
...
VAddr GlobalStubs::GetAbiHostToGuest() const {
    return abi_switch_host_to_guest_;
}
...
void GlobalStubs::BuildForwardCodeCache() {
    ...
    // load rt
    __ Ldr(rt, MemOperand(context_reg_, OFFSET_CTX_A64_FORWARD));
    // Find hash table
    ...
    __ Ldp(tmp1, tmp2, MemOperand(context_reg_, tmp1.RealCode() * 8));
    __ Ldr(forward_reg_, MemOperand(context_reg_, forward_reg_.RealCode() * 8));
    ABISaveContext(masm_, tmp1);
    __ Mov(forward_reg_, (VAddr)CodeCacheMissStub);
    __ Mov(x0, context_reg_);
    __ Blr(forward_reg_);
    __ Mov(context_reg_, x0);
    ABIRestoreContext(masm_, const_cast<Register &>(forward_reg_));
    __ Ldr(forward_reg_, MemOperand(context_reg_, OFFSET_CTX_A64_FORWARD));
    __ Br(forward_reg_);
...
void GlobalStubs::ABISaveContext(MacroAssembler &masm_, Register &tmp) {
    // x regs
...
void GlobalStubs::ABIRestoreContext(MacroAssembler &masm_, Register &tmp) {
    for (int i = 0; i < 19; i += 2) {
        if (i == context_reg_.RealCode()) {
            __ Ldr(XRegister::GetXRegFromCode(i + 1),
                   MemOperand(context_reg_, 8 * (i + 1)));
...
void GlobalStubs::BuildABISwitchGuestToHost() {
    MacroAssembler masm_;
    auto tmp = forward_reg_;
    // restore forward reg first
    __ Ldr(forward_reg_, MemOperand(context_reg_, forward_reg_.RealCode() * 8));

    ABISaveContext(masm_, tmp);
    ...
    auto stub_size = __ GetBuffer()->GetSizeInBytes();
    VAddr buffer_start = abi_switch_guest_to_host_;
    VAddr tmp_code_start = __ GetBuffer()->GetStartAddress<VAddr>();
    __ FinalizeCode();
    std::memcpy(reinterpret_cast<void *>(buffer_start),
                reinterpret_cast<const void *>(tmp_code_start), stub_size);
...
void GlobalStubs::BuildABISwitchHostToGuest() {
    MacroAssembler masm_;
    auto tmp = forward_reg_;
    Label code_lookup_label;
    ...
    auto stub_size = __ GetBuffer()->GetSizeInBytes();
    VAddr buffer_start = abi_switch_host_to_guest_;
    VAddr tmp_code_start = __ GetBuffer()->GetStartAddress<VAddr>();
    __ FinalizeCode();
    std::memcpy(reinterpret_cast<void *>(buffer_start),
                reinterpret_cast<const void *>(tmp_code_start), stub_size);
